...
async def insta_command_handler(event):
    ...
    try:
        ...
        await event.edit(f"**✅ Rest Done For **`{email_or_username}`\n**✅ Result:**\n```{result}```\n**[SHRIDHAR]**(tg://openmessage?user_id=6432270973)")
    except Exception as e:
...
async def dm_user_handler(event):
    ...
    if replied_user:
        ...
        mentioned_user = event.pattern_match.group(1)
        if mentioned_user:
            try:
                if mentioned_user.startswith('@'):
                    user = await client.get_entity(mentioned_user)
                else:
                    user = await client.get_entity(int(mentioned_user))
            except Exception as e:
